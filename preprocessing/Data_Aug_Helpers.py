...
def alignFace(img_tensor):
	# img = cv2.imread(img_path)
	# plt.imshow(img[:, :, ::-1])
	# plt.show()

    pil_image = Image.fromarray(np.asarray(img_tensor)).convert('RGB') 
    ...
    if len(eyes) >= 2:
        ...
        if left_eye_y > right_eye_y:
            ...
            print("rotate to clockwise direction")
        else:
            point_3rd = (left_eye_x, right_eye_y)
            direction = 1 #rotate inverse direction of clock
            print("rotate to anti-clockwise direction")
